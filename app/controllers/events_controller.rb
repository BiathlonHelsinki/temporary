...
#     end
  ...
  def index
    ...
    # @experiments = Instance.temporary.includes(:translations).where(place_id: 1).current.published.or(Instance.future.published.includes(:translations)).order(:start_at).uniq
    # @past = Instance.temporary.includes(%i[translations users onetimers]).where(place_id: 1).past.published.order(start_at: :desc).limit(8).uniq
  ...
  def set_item
    @experiment = Event.friendly.find(params[:id])
    redirect_to(action: action_name, id: @experiment.friendly_id, status: :moved_permanently) unless @experiment.friendly_id == params[:id]
  end
end
