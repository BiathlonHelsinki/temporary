class Activity < ApplicationRecord
  ...
  def linked_name
    ...
    when 'User'
      if value
        "#{item.display_name}  <br /><small>#{t(extra_info.to_sym)}</small>"
      elsif extra
        "<a href='/users/" + item.slug + "'>" + item.display_name + "</a> #{t(extra_info.to_sym)} <a href='/" + extra.class.table_name + "/#{extra.id.to_s}'>" + extra.name + "</a>"
